...
img_path = "img/img7.jpg"
image = cv2.imread(img_path)  
hsv = cv2.cvtColor(image, cv2.COLOR_BGR2HSV)
h, s, v = cv2.split(hsv)
v = np.clip(v + 5, 0, 255)  # brighten highlights
final_hsv = cv2.merge((h, s, v))
highlight_boosted = cv2.cvtColor(final_hsv, cv2.COLOR_HSV2BGR)


blur = cv2.GaussianBlur(highlight_boosted, (7, 7), 1)
...
merged_corners = merge_close_points(corner_coords, threshold=20)
...
# Mask corner points
for (x, y) in merged_corners:
    cv2.circle(inverted, (x, y), radius=15, color=0, thickness=-1)
...
def detect_lines_and_circles(image):
    ...
    if contours:
        ...
        for cnt in contours:
            ...
            if aspect_ratio > 2 or (aspect_ratio > 1 and area < 200) :
...
def draw_polygons_from_lines(image):
    ...
    newPolygonList = copy.deepcopy(polygonList)

    for polygon in newPolygonList:
        ...
        while i < len(polygon):
            ...
            if (abs(slope1 - slope2) < 0.1):
                ...
                print("------FLAG---------")
                continue
            intersect_pt = line_intersection(line1[0], line1[1], line2[0], line2[1])
            if(intersect_pt):
                px, py = intersect_pt
    ...
    texts = extract_texts_from_polygons(img_path, polygonsVertexList)
    # texts = []
    print(texts)
    ...
    def is_point_outside_all_polygons(point, polygons):
        for poly in polygons:
            contour = np.array(poly, dtype=np.int32)
            inside = cv2.pointPolygonTest(contour, point, measureDist=False)
            if inside >= 0:  # point is inside or on the edge
                return False
        return True  # Point is outside all polygons

    for line in connectionLines:
        pt1, pt2 = line
        isOutSidePolygon = is_point_outside_all_polygons(pt1, polygonsVertexList) and is_point_outside_all_polygons(pt2, polygonsVertexList)
        if isOutSidePolygon:
